...
Each function requires and ensures `thread_token(t)`, providing access to resources accessible to thread `t`, where `t` denotes the current thread. The completed versions of the examples above are at [`full_borrow_tutorial.rs`](https://github.com/verifast/verifast/blob/master/tests/rust/safe_abstraction/full_borrow_tutorial.rs) and [`frac_borrow_tutorial.rs`](https://github.com/verifast/verifast/blob/master/tests/rust/safe_abstraction/frac_borrow_tutorial.rs).
