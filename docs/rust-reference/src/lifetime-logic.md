...
Above, we ignored the fact that some Rust values (of non-Send types), such as [`&Cell`](https://doc.rust-lang.org/std/cell/struct.Cell.html) values and [`Rc`](https://doc.rust-lang.org/std/rc/struct.Rc.html) values are accessible only to particular threads. To model this, each type's `own` predicate takes a thread identifier: `<T>.own(t, v)` asserts ownership of Rust value `v` of type T and that `v` is accessible to thread `t`. Similarly, `<T>.share(k, t, l)` asserts shared ownership of the place of type T at `l` and the value stored at that place, accessible to thread `t`, until the end of lifetime `k`.

Each function requires and ensures `thread_token(t)`, providing access to resources accessible to thread `t`, where `t` denotes the current thread. The completed versions of the examples above are at [`full_borrow_tutorial.rs`](https://github.com/verifast/verifast/blob/master/tests/rust/safe_abstraction/full_bor_primitive_types.rs) and [`frac_borrow_tutorial.rs`](https://github.com/verifast/verifast/blob/master/tests/rust/safe_abstraction/frac_borrow_tutorial.rs).