...
void writer_write_string(struct writer *writer, char *string);
    //@ requires writer(writer) &*& [?f]chars(string, ?n, ?cs) &*& mem('\0', cs) == true;
    //@ ensures writer(writer) &*& [f]chars(string, n, cs);
...
char *reader_read_line_as_string(struct reader *reader);
    //@ requires reader(reader);
    //@ ensures reader(reader) &*& result == 0 ? true : chars(result, ?n, ?cs) &*& mem('\0', cs) == true &*& malloc_block(result, length(cs));
