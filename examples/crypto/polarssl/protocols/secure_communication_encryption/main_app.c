...
/*@
predicate sc_enc_proof_pred() = true;

predicate_family_instance pthread_run_pre(attacker_t)(void *data, any info) =
    [1/3]polarssl_world(sc_enc_polarssl_pub) &*&
    polarssl_proof_obligations(sc_enc_polarssl_pub, sc_enc_proof_pred) &*&
  ...
  //@ ensures  false;
{
    ...
    //@ invariant pthread_run_pre(attacker_t)(data, info);
  {
    ...
    //@ close sc_enc_proof_pred();
    polarssl_attacker(data);
    //@ open sc_enc_proof_pred();
...
predicate_family_instance pthread_run_pre(sender_t)(void *data, any info) =
  ...
  [1/2]polarssl_cryptogram(key, KEY_SIZE, ?key_cs, ?key_cg) &*&
  ...
  [1/2]chars(message, message_len, ?msg_cs) &*&
    ...
    (bad(sender) ?
       [_]polarssl_public_generated_chars(sc_enc_polarssl_pub)(msg_cs)
     :
...
{
  //@ open pthread_run_pre(sender_t)(data, info);
  struct ss_args *args = data;
  app_send(args->key, args->message, args->message_len);
  //@ close pthread_run_post(sender_t)(data, info);
  return 0;
}
  ...
  //@ ensures  true;
{
  ...
  *principals = 0;
  ...
  printf("\n\tExecuting \""); 
  printf("secure_communication");
  printf("_encryption");
  printf(" protocol");
  printf("\" ... \n\n");
  
  
  //@ PACK_PROOF_OBLIGATIONS(sc_enc)
  //@ close exists(sc_enc_polarssl_pub);
  //@ polarssl_init();
  ...
  while (true)
#endif
    /*@ invariant [2/3]polarssl_world(sc_enc_polarssl_pub) &*& 
    ...
    @*/
  {
    ...
    if (havege_random(&havege_state, message, (unsigned int) message_len) != 0) 
      abort();
    //@ open polarssl_cryptogram(message, message_len, ?msg_cs, ?msg_cg);
    //@ assume (!bad(sender));
    //@ assume (app_send_event(sender, msg_cs));
    //@ polarssl_public_generated_chars_assume(sc_enc_polarssl_pub, msg_cs);
    {
      ...
      /*@ open polarssl_cryptogram(key, KEY_SIZE, ?key_cs,
                               polarssl_symmetric_key(sender, count_s + 1)); @*/
      //@ assert chars(key, KEY_SIZE, ?cs_key);
      s_args.key = key;
      s_args.message_len = message_len;
      s_args.message = message;

      r_args.key = key;
      /*@ close polarssl_cryptogram(key, KEY_SIZE, key_cs,
                               polarssl_symmetric_key(sender, count_s + 1)); @*/
