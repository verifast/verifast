...
predicate_family_instance pthread_run_pre(server_t)(void *data, any info) =
  ...
  item(key_AB, key_item(server_id(), 1, symmetric_key, int_pair(2, int_pair(sender, receiver)))) &*&
  info == nil;
  ...
  //@ ensures  true;
{
  ...
  //@ close key_request(server, int_pair(2, int_pair(sender, receiver)));
  ...
  while (true)
    ...
    /*@ invariant
              [_]world(yahalom_pub) &*&
              generated_values(sender, _) &*&
              item(key_AS, ?kkey_AS) &*& kkey_AS ==
                key_item(sender, 1, symmetric_key, int_pair(0, 0)) &*&
              generated_values(receiver, _) &*&
              item(key_BS, ?kkey_BS) &*& kkey_BS ==
                key_item(receiver, 1, symmetric_key, int_pair(0, 0)) &*&
              generated_values(server, _) &*&
              item(key_AB, ?kkey_AB) &*& kkey_AB == 
                key_item(server, 1, symmetric_key, int_pair(2, int_pair(sender, receiver)));
    @*/
  {
