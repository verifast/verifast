             ...
             [f2]crypto_chars(secret, msg, msg_len, msg_cs); @*/
{
  ...
  
  {
    ...
    //@ assert crypto_chars(secret, (void*) message + 16, 16, ?tag_cs);
    //@ assert crypto_chars(secret, (void*) message + 32, msg_len, ?enc_cs);
    //@ crypto_chars_join(message + 16);
    //@ close cryptogram(message + 16, 16 + msg_len, append(tag_cs, enc_cs), enc_cg);
    //@ assert enc_cg == cg_auth_encrypted(sender, id, msg_cs, iv_cs);
    //@ close auth_enc_pub(enc_cg);
    //@ leak auth_enc_pub(enc_cg);
    //@ public_cryptogram(message + 16, enc_cg);
    //@ chars_join(message);
    /*@ assert chars(message, 16 + msg_len + 16, 
                     append(iv_cs, append(tag_cs, enc_cs))); @*/
               ...
               send(sender, receiver, msg_cs); @*/
{
  ...
  int enc_size;
  {
    ...
    //@ open cryptogram(buffer + 16, 16 + enc_size, _, ?enc_cg);
    //@ close exists(enc_cg);
    ...
    /*@ if (!col)
        {
          assert enc_cg == cg_auth_encrypted(sender, id, ?dec_cs, iv_cs);
          assert chars_for_cg(enc_cg) == append(tag_cs, enc_cs);
