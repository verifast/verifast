...
predicate_family_instance pthread_run_pre(sender_t)(void *data, any info) =
  ...
  principal(sender, _) &*&
  [1/2]cryptogram(enc_key1, KEY_SIZE, ?enc_key_cs1, ?enc_key_cg1) &*&
    enc_key_cg1 == cg_symmetric_key(sender, ?enc_id1) &*&
    receiver == shared_with(sender, enc_id1) &*&
  [1/2]cryptogram(enc_key2, KEY_SIZE, ?enc_key_cs2, ?enc_key_cg2) &*&
  ...
  info == IV(sender, IV(receiver, PV(enc_key1, CL(enc_key_cs1, IV(enc_id1,
             PV(enc_key2, CL(enc_key_cs2, IV(enc_id2, PV(hmac_key, 
                CL(hmac_key_cs, IV(hmac_id, PV(msg, CL(msg_cs, nil)))))))))))));
...
predicate_family_instance pthread_run_pre(receiver_t)(void *data, any info) =
  ...
  info == IV(sender, IV(receiver, PV(enc_key1, CL(enc_key_cs1, IV(enc_id1,
             PV(enc_key2, CL(enc_key_cs2, IV(enc_id2, PV(hmac_key, 
                CL(hmac_key_cs, IV(hmac_id, PV(msg, nil))))))))))));
