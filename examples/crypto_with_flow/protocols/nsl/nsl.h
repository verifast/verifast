...
predicate nsl_pub_msg2(int sender, int receiver, cryptogram s_nonce,
                       list<char> s_nonce_cs, cryptogram r_nonce, int p_inst,
                       int p_orig, int c_orig, bool pub_ns) = true;
  ...
  switch (cg)
  {
    ...
    case cg_asym_encrypted(p0, c0, cs0, ent0):
      ...
      : length(cs0) == MSG2_SIZE ?
        (
          // 2. B -> A. {B,NA,NB}_K(A)
          nsl_pub_msg2(?sender, ?receiver, ?s_nonce, ?s_nonce_cs, ?r_nonce,
