#include "sign.h"

#include <stdlib.h>
#include <string.h>

//@ #include "quantifiers.gh"

#define SERVER_PORT 121212

int random_stub_sign(void *havege_state, char *output, size_t len)
  //@ requires PRG_PRECONDITION(havege_state_initialized, havege_state);
  //@ ensures PRG_POSTCONDITION(havege_state_initialized, havege_state);
{
  return havege_random(havege_state, output, len);
}

void sender(int recvr, char *key, int key_len, char *msg)
  /*@ requires [_]public_invar(sign_pub) &*&
               principal(?sender, _) &*&
               [?f1]cryptogram(key, key_len, ?key_cs, ?key_cg) &*&
                 key_cg == cg_private_key(sender, ?id) &*&
                 key_len >= 384 &*& key_len < MAX_KEY_SIZE &*&
               [?f2]chars(msg, MSG_SIZE, ?msg_cs) &*&               
               true == send(sender, recvr, msg_cs); @*/
  /*@ ensures  principal(sender, _) &*&
               [f1]cryptogram(key, key_len, key_cs, key_cg) &*&
               [f2]chars(msg, MSG_SIZE, msg_cs); @*/
{
  int socket;
  char hash[48];
  pk_context context;
  havege_state havege_state;
  
  net_usleep(20000);
  if(net_connect(&socket, NULL, SERVER_PORT) != 0)
    abort();
  if(net_set_block(socket) != 0)
    abort();
  
  {
    unsigned int sign_len;
    int max_message_len = 4 + MSG_SIZE + key_len;
    char* M = malloc(max_message_len);
    if (M == 0) abort();
    
    // fill in plain text
    //@ assert integer(&recvr, ?receiver);
    //@ integer_to_chars(&recvr);
    /*@ close optional_crypto_chars(false, (void*) &recvr, 
                                    4, chars_of_int(receiver)); @*/
    memcpy(M, &recvr, 4);
    /*@ open optional_crypto_chars(false, (void*) &recvr, 
                                   4, chars_of_int(receiver)); @*/
    //@ open optional_crypto_chars(false, M, 4, chars_of_int(receiver));
    //@ chars_to_integer(&recvr);
    //@ close [f2]optional_crypto_chars(false, msg, MSG_SIZE, msg_cs);
    memcpy(M + 4, msg, (unsigned int) MSG_SIZE);
    //@ open [f2]optional_crypto_chars(false, msg, MSG_SIZE, msg_cs);
    //@ open optional_crypto_chars(false, M + 4, MSG_SIZE, msg_cs);
    //@ chars_join(M);
    //@ list<char> pay = append(chars_of_int(receiver), msg_cs);
    
    // create hash of plain text
    //@ assert chars(M, 4 + MSG_SIZE, pay);
    //@ close optional_crypto_chars(false, M, 4 + MSG_SIZE, pay);
    sha512(M, (unsigned int) (4 + MSG_SIZE), hash, 1);
    //@ open optional_crypto_chars(false, M, 4 + MSG_SIZE, pay);
    //@ assert cryptogram(hash, 48, ?hash_cs, ?hash_cg);
    //@ close sign_pub(hash_cg);
    //@ leak sign_pub(hash_cg);
    //@ public_cryptogram(hash, hash_cg);
    //@ close optional_crypto_chars(false, hash, 48, hash_cs);
    
    // create signature
    //@ close pk_context(&context);
    pk_init(&context);
    if (pk_parse_key(&context, key, (unsigned int) key_len, NULL, 0) != 0)
      abort();
    //@ close havege_state(&havege_state);
    havege_init(&havege_state);
    //@ close random_state_predicate(havege_state_initialized);
    /*@ produce_function_pointer_chunk random_function(random_stub_sign)
                     (havege_state_initialized)(state, out, len) { call(); } @*/    
    if (pk_sign(&context, POLARSSL_MD_NONE, hash, 48, M + 4 + MSG_SIZE, 
                &sign_len, random_stub_sign, &havege_state) != 0)
      abort();
    //@ pk_release_context_with_key(&context);
    pk_free(&context);
    //@ open pk_context(&context);
    havege_free(&havege_state);
    //@ open havege_state(&havege_state);
    //@ open optional_crypto_chars(false, hash, 48, hash_cs);
    //@ assert u_integer(&sign_len, ?sign_len_val);
    //@ assert cryptogram(M + 4 + MSG_SIZE, sign_len_val, ?cs_sign, ?cg_sign);
    //@ assert chars(M + 4 + MSG_SIZE + sign_len_val, key_len - sign_len_val, _);
    //@ assert cg_sign == cg_asym_signature(sender, id, hash_cs, _);
    //@ if (!bad(sender)) close sign_pub_1(msg_cs, recvr);    
    //@ close sign_pub(cg_sign);
    //@ leak sign_pub(cg_sign);
    //@ public_cryptogram(M + 4 + MSG_SIZE, cg_sign);
    
    int message_len = 4 + MSG_SIZE + (int) sign_len;
    //@ chars_join(M);
    net_send(&socket, M, (unsigned int) message_len);
    //@ chars_join(M);
    free(M);
  }
  net_close(socket);
}

void receiver(int recvr, char *key, int key_len, char *msg)
  /*@ requires [_]public_invar(sign_pub) &*&
               principal(recvr, _) &*&
               [?f1]cryptogram(key, key_len, ?key_cs, ?key_cg) &*&
                 key_cg == cg_public_key(?sender, ?id) &*&
                 key_len <= MAX_KEY_SIZE &*&
               chars(msg, MSG_SIZE, _); @*/
  /*@ ensures  principal(recvr, _) &*&
               [f1]cryptogram(key, key_len, key_cs, key_cg) &*&
               chars(msg, MSG_SIZE, ?msg_cs) &*&
               bad(sender) || collision_in_run() ||
                 send(sender, recvr, msg_cs); @*/
{
  int socket1;
  int socket2;
  
  if(net_bind(&socket1, NULL, SERVER_PORT) != 0)
    abort();
  if(net_accept(socket1, &socket2, NULL) != 0)
    abort();
  if(net_set_block(socket2) != 0)
    abort();
  
  {
    pk_context context;
    int max_size = 4 + MSG_SIZE + MAX_KEY_SIZE;
    char hash[48];
    int* temp;
    char *buffer = malloc (max_size); if (buffer == 0) abort();
    int size = net_recv(&socket2, buffer, (unsigned int) max_size);
    if (size <= 4 + MSG_SIZE) abort();
    if (48 * 8 > key_len) abort();
    
    //@ chars_split(buffer, size);
    //@ assert chars(buffer, size, ?all_cs);
    //@ close hide_chars((void*) buffer + size, max_size - size, _);
    
    // Parse plain text
    //@ chars_split(buffer, 4);
    //@ chars_to_integer(buffer);
    temp = (void*) buffer;
    int receiver = *temp;
    if (recvr != receiver) abort();
    //@ integer_to_chars(buffer);
    //@ assert chars(buffer, 4, chars_of_int(receiver));
    //@ chars_split(buffer + 4, MSG_SIZE);
    //@ close optional_crypto_chars(false, buffer + 4, MSG_SIZE, ?msg_cs);
    memcpy(msg, buffer + 4, MSG_SIZE);
    //@ open optional_crypto_chars(false, buffer + 4, MSG_SIZE, msg_cs);
    int sign_size = size - 4 - MSG_SIZE;
    //@ assert chars(buffer + 4 + MSG_SIZE, sign_size, ?sign_cs);
    
    // Calculate hash
    //@ chars_join(buffer);
    //@ close optional_crypto_chars(false, buffer, 4 + MSG_SIZE, ?pay);
    //@ assert pay == append(chars_of_int(receiver), msg_cs);
    sha512(buffer, (unsigned int) (4 + MSG_SIZE), hash, 1);
    //@ open optional_crypto_chars(false, buffer, 4 + MSG_SIZE, pay);
    //@ assert cryptogram(hash, 48, ?hash_cs, ?hash_cg);
    //@ close sign_pub(hash_cg);
    //@ leak sign_pub(hash_cg);
    //@ public_cryptogram(hash, hash_cg);
    //@ close optional_crypto_chars(false, hash, 48, hash_cs);
    
    // Check signatrure
    //@ close pk_context(&context);
    pk_init(&context);
    if (pk_parse_public_key(&context, key, (unsigned int) key_len) != 0)
      abort();  
    //@ close optional_crypto_chars(false, buffer + 4 + MSG_SIZE, sign_size, sign_cs);
    if (pk_verify(&context, POLARSSL_MD_NONE, hash, 48, 
                  buffer + 4 + MSG_SIZE, (unsigned int) sign_size) != 0)
      abort();
    //@ open optional_crypto_chars(false, buffer + 4 + MSG_SIZE, sign_size, sign_cs);
    //@ pk_release_context_with_key(&context);
    pk_free(&context);
    //@ open pk_context(&context);
    
    //@ assert exists(?ent);
    //@ cryptogram sign_cg1 = cg_asym_signature(sender, id, hash_cs, ent);
    //@ assert sign_cs == chars_for_cg(sign_cg1);
    //@ public_chars_extract(buffer + 4 + MSG_SIZE, sign_cg1);
    //@ open [_]sign_pub(sign_cg1);
    /*@ if (!bad(sender) && !collision_in_run())
        {
          assert [_]sign_pub_1(?msg_cs2, ?receiver2);
          cryptogram hash_cg2 = cg_hash(append(chars_of_int(receiver2), msg_cs2));
          chars_for_cg_inj(hash_cg, hash_cg2);
          assert true == send(sender, receiver2, msg_cs2);
          drop_append(4, chars_of_int(receiver), msg_cs);
          drop_append(4, chars_of_int(receiver2), msg_cs2);
          take_append(4, chars_of_int(receiver), msg_cs);
          take_append(4, chars_of_int(receiver2), msg_cs2);
          chars_of_int_injective(receiver, receiver2);
          assert true == send(sender, receiver, msg_cs);
        }
    @*/
    
    //@ chars_join(buffer);
    //@ append_assoc(chars_of_int(receiver), msg_cs, sign_cs);
    //@ assert chars(buffer, size, append(pay, sign_cs));
    //@ open hide_chars((void*) buffer + size, max_size - size, _);    
    free(buffer);
  }
  net_close(socket2);
  net_close(socket1);
}
