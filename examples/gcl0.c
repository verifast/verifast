...
struct atom *create_atom_from_string(char *string)
    //@ requires globals(?objects0, ?roots) &*& [?f]chars(string, ?cs) &*& mem('\0', cs) == true;
    //@ ensures globals(?objects1, roots) &*& [f]chars(string, cs) &*& mem((void *)result, objects1) == true;
{
...
void map_cons_s(char *key, struct object *value, struct object **map)
    //@ requires globals(?objects, ?roots) &*& [?f]chars(key, ?cs) &*& mem('\0', cs) == true &*& mem(value, objects) == true &*& [1/2]pointer(map, _) &*& mem(map, roots) == true;
    //@ ensures globals(_, roots) &*& [f]chars(key, cs) &*& [1/2]pointer(map, _);
{
    struct atom *atom;
    void *result;
...
void map_cons_s_func_nil(char *key, apply_func *function, struct object **map)
    //@ requires globals(?objects, ?roots) &*& [?f]chars(key, ?cs) &*& mem('\0', cs) == true &*& is_apply_func(function) == true &*& [1/2]pointer(map, _) &*& mem(map, roots) == true;
    //@ ensures globals(_, roots) &*& [f]chars(key, cs) &*& [1/2]pointer(map, _);
{
