...
class Util {
    ...
    static int sum(Func f1, Func f2, int x)
        //@ requires [_]Func(f1, ?f1Level) &*& [_]Func(f2, ?f2Level) &*& call_perm(cons(Util.class, level_max(f1Level, f2Level)));
        //@ ensures true;
        //@ terminates;
    {
        //@ call_perm_weaken_and_dup(4);
    ...
    static int double_(Func f, int x)
        //@ requires [_]Func(f, ?fLevel) &*& call_perm(cons(Util.class, fLevel));
        //@ ensures true;
        //@ terminates;
    {
        return sum(f, f, x);
...
final class SumFunc implements Func {
    ...
    public static Func create(Func f1, Func f2)
        //@ requires [_]Func(f1, ?f1Level) &*& [_]Func(f2, ?f2Level) &*& call_perm(cons(SumFunc.class, level_max(f1Level, f2Level)));
