...
struct atom *create_atom_from_string(char *string)
    //@ requires heap() &*& [?f]chars(string, ?cs) &*& mem('\0', cs) == true;
    //@ ensures heap() &*& [f]chars(string, cs) &*& ref((void *)result);
{
...
struct object *map_cons_s(char *key, struct object *value, struct object *map)
    //@ requires heap() &*& [?f]chars(key, ?cs) &*& mem('\0', cs) == true &*& ref(value) &*& ref(map);
    //@ ensures heap() &*& [f]chars(key, cs) &*& ref(result);
{
...
struct object *map_cons_s_func_nil(char *key, apply_func *function, struct object *map)
    //@ requires heap() &*& [?f]chars(key, ?cs) &*& mem('\0', cs) == true &*& is_apply_func(function) == true &*& ref(map);
    //@ ensures heap() &*& [f]chars(key, cs) &*& ref(result);
{
