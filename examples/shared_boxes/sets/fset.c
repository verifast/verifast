...
/*
   note about cap:
   - the cap paper every now and they seems read a value without permission. Maybe this is because they leave out an 
   ...
   - the cap paper can use existential quantifiers in the actions, and therefore leave certain things unspecified
     with shared boxes, patterns are not supported (neither in the definition or as arguments when performing the action).
...
predicate node_internal(struct llnode *node; struct mutex3 *mutex, int value) =
    node != 0 &*&
    malloc_block_llnode_ex(node) &*&
    [1/2]node->value |-> value &*&
    [1/3]node->mutex |-> mutex;
    
predicate link(struct llnode *node; struct llnode *next) = 
    node != 0 &*&
    [1/3]node->mutex |-> ?mutex &*&
    node->next |-> next &*& 
    (next == 0 ? true : [1/3]next->mutex |-> ?nmutex &*& mutex3(nmutex) &*& [1/2]next->value |-> ?value);

predicate linkex(struct llnode *node; struct mutex3* mutex, struct llnode *next, struct mutex3* nmutex, int nvalue) = 
    node != 0 &*&
    [1/3]node->mutex |-> mutex &*&
    node->next |-> next &*& 
    next != 0 &*& 
    [1/3]next->mutex |-> nmutex &*& 
    mutex3(nmutex) &*& 
    [1/2]next->value |-> nvalue;

lemma void linkex_distinct(struct llnode *n1, struct llnode *n2) 
    requires linkex(n1, ?n1m, ?n1n, ?n1nm, ?n1nv) &*& linkex(n2, ?n2m, ?n2n, ?n2nm, ?n2nv);
    ensures linkex(n1, n1m, n1n, n1nm, n1nv) &*& linkex(n2, n2m, n2n, n2nm, n2nv) &*& n1 != n2;
{
    open linkex(n1, n1m, n1n, n1nm, n1nv);
    open linkex(n2, n2m, n2n, n2nm, n2nv);
    close linkex(n1, n1m, n1n, n1nm, n1nv);
    close linkex(n2, n2m, n2n, n2nm, n2nv);
}

predicate linkex2(struct llnode *node; struct mutex3* mutex, struct llnode *next) = 
    node != 0 &*&
    [1/3]node->mutex |-> mutex &*&
    node->next |-> next &*& 
    (next == 0 ? true : [1/3]next->mutex |-> ?nmutex &*& mutex3(nmutex) &*& [1/2]next->value |-> ?value);
...
predicate freenode(struct llnode *node; struct llnode *next, int value) =
    node_internal(node, ?mutex, value) &*&
    link(node, next);

predicate lockednode(struct llnode *node; int value) =
    node_internal(node, ?mutex, value) &*&
    mutex3_held(mutex);

predicate unknownnode(struct llnode *node, struct llnode *next, int value, bool locked) =
    (locked ? lockednode(node, value) : freenode(node, next, value));
    ...
    ensures unknownnode(node, next, value, locked) &*& node != 0;
{
    ...
    open node_internal(node, ?m, value);
    close node_internal(node, m, value);
    close unknownnode(node, next, value, locked);
}

lemma void convert_unknownnode_mutexheld(struct llnode *node) 
   requires [?f]node->mutex |-> ?mutex &*& mutex3_held(mutex) &*& unknownnode(node, ?next, ?value, ?locked);
   ensures [f]node->mutex |-> mutex &*& mutex3_held(mutex) &*& unknownnode(node, next, value, locked) &*& !locked;
{
    open unknownnode(node, next, value, locked);
    open node_internal(node, _, value);
    close node_internal(node, _, value);
    if(locked) mutex3_exclusive(mutex);
    close unknownnode(node, next, value, locked);
}
lemma void lock_freenode(struct llnode *node) 
    requires [?f]node->mutex |-> ?mutex &*& mutex3_held(mutex) &*& freenode(node, ?next, ?value);
    ensures [f]node->mutex |-> mutex &*& lockednode(node, value) &*& link(node, next);
{
    open freenode(node, next, value);
    open node_internal(node, _, value);
    close node_internal(node, mutex, value);
}
lemma void unlock_lockednode(struct llnode *node, struct llnode *next, int value) 
    requires linkex2(node, ?mutex, next) &*& lockednode(node, value);
    ensures mutex3_held(mutex) &*& freenode(node, next, value);
{
    open linkex2(node, mutex, next);
    open lockednode(node, value);
    open node_internal(node, _, value);
    close node_internal(node, mutex, value);
    close link(node, next);
    close freenode(node, next, value);
}


lemma void node_internal_distinct(struct llnode *n1, struct llnode *n2)
    requires node_internal(n1, ?m1, ?v1) &*& node_internal(n2, ?m2, ?v2);
    ensures node_internal(n1, m1, v1) &*& node_internal(n2, m2, v2) &*& n1 != n2;
{
    open node_internal(n1, m1, v1);
    open node_internal(n2, m2, v2);
    malloc_block_llnode_ex_distinct(n1, n2);
    close node_internal(n1, m1, v1);
    close node_internal(n2, m2, v2);
}
    ...
    ensures unknownnode(node1, next1, value1, locked1) &*& unknownnode(node2, next2, value2, locked2) &*& node1 != node2;
{
    ...
    if(locked1) open lockednode(node1, value1); else open freenode(node1, next1, value1);
    open unknownnode(node2, next2, value2, locked2);
    if(locked2) open lockednode(node2, value2); else open freenode(node2, next2, value2);
    node_internal_distinct(node1, node2);
    if(locked1) close lockednode(node1, value1); else close freenode(node1, next1, value1);
    close unknownnode(node1, next1, value1, locked1);
    if(locked2) close lockednode(node2, value2); else close freenode(node2, next2, value2);
        ...
        return switch(gn) {
            case gnode(node, next, value, owner):
                return 
                    node == from &*& 
                    from != to &*&
                    unknownnode(node, next, value, owner != INT_MIN) &*&
                    lsg(next, to, gnodes2);
        };
    };
...
lemma void lsg_openex(struct llnode* from, struct llnode* to, list<gnode> gns) 
    ...
    ensures gres(?owner) &*& unknownnode(from, ?next, ?value, owner != INT_MIN) &*& lsg(next, to, ?gns2) &*& 
...
lemma void lsg_locknode(struct llnode *from, list<gnode> gnodes, struct llnode *node, int owner)
    ...
    ensures 
        [f]node->mutex |-> mutex &*& 
        [f2]node->value |-> value &*&
        linkex(node, mutex, ?next, ?nextmutex, ?nextvalue) &*&
    ...
    case cons(gn, gnodes2):
        ...
        case gnode(gnnode, gnnext, gnvalue, gnowner):
            ...
            if(from == node) {
                ...
                open unknownnode(node, gnnext, gnvalue, false);
                lock_freenode(node);
                
                open node_internal(node, ?nodemutex, gnvalue);
                assert gnvalue == value;
                close node_internal(node, mutex, value);
                
                close unknownnode(node, gnnext, gnvalue, true);
                ...
                open link(node, gnnext);
                close linkex(node, mutex, gnnext, _, _);
                ...
                assert linkex(node, mutex, ?next, ?nmutex, ?nvalue);
                if(gnowner == owner) {
    ...
    switch(gnodes) {
        ...
        switch(gn) {
            ...
            if(gnnode == node) {
                ...
                assert gnnext == next &*& gnvalue == value &*& gnowner == owner;
                open linkex(node, mutex, next, nextmutex, nextvalue);
                close linkex2(node, mutex, next);
                open unknownnode(node, next, value, true);
                unlock_lockednode(node, next, value);
                close unknownnode(node, next, value, false);
...
lemma void lsg_contains(struct llnode* from, list<gnode> gnodes, list<int> values, struct llnode* p, int pv, int value)
    requires lsg(from, 0, gnodes) &*& lgn_values(gnodes) == cons(INT_MIN, snoc(values, INT_MAX)) &*& sorted(lgn_values(gnodes)) == true &*& 
        linkex(p, ?pm, ?c, ?cm, ?cv) &*& mem(gnode(p, c, pv, value), gnodes) == true &*& pv < value &*& value <= cv &*& INT_MIN < value &*& value < INT_MAX;
    ensures lsg(from, 0, gnodes) &*& linkex(p, pm, c, cm, cv) &*& mem(value, values) == (value == cv);
{
    close lgn_contains(gnodes, p, c, pv, value);
    lsg_split(from, gnodes, p);
    assert lsg(from, p, ?gns1) &*& unknownnode(p, c, pv, true) &*& lsg(c, 0, ?gns2);
    open linkex(p, pm, c, cm, cv);
    open lsg(c, 0, gns2);
    assert c != 0;
    switch(gns2) {
        case nil:
        ...
        switch(gn) {
            case gnode(gnnode, gnnext, gnvalue, gnowner):
                assert gnnode == c;
                open unknownnode(c, gnnext, gnvalue, gnowner != INT_MIN);
                if(gnowner == INT_MIN) open freenode(c, gnnext, gnvalue); else open lockednode(c, gnvalue);
                open node_internal(c, ?cm2, gnvalue);
                close linkex(p, pm, c, cm, cv);
                close node_internal(c, cm, cv);
                if(gnowner == INT_MIN) close freenode(c, gnnext, cv); else close lockednode(c, cv);
                close unknownnode(c, gnnext, cv, gnowner != INT_MIN);
                close lsg(c, 0, gns2);
...
lemma void lsg_gap2_helper(struct llnode *from, list<gnode> gnodes, struct llnode *p, int pvalue, struct llnode *c, int v)
    requires 
        lsg(from, 0, gnodes) &*& 
        linkex(p, ?pmutex, ?n, ?nmutex, v) &*& 
        node_internal(n, nmutex, v) &*& 
        linkex(n, nmutex, c, ?cmutex, ?cvalue) &*& 
    ...
    switch(gnodes) {
        ...
        switch(gn) {
            ...
            if(gnnode == p) {
                ...
                open unknownnode(p, c, pvalue, true);
                open node_internal(p, _, pvalue);
                open linkex(p, pmutex, n, nmutex, v);
                close node_internal(p, pmutex, pvalue);
                close unknownnode(p, n, pvalue, false);
                assert mutex3_held(pmutex);
                open linkex(n, nmutex, c, cmutex, cvalue);
                ...
                switch(gnodes2) {
                    ...
                    switch(gn2) {
                        ...
                        assert gn2node == c;
                        open unknownnode(c, gn2next, gn2value, gn2owner != INT_MIN);
                        open node_internal(c, _, gn2value);
                        close node_internal(c, cmutex, cvalue);
                        close unknownnode(c, gn2next, cvalue, gn2owner != INT_MIN);
                        assert gn2value == cvalue;
                        close link(n, c);
                        close freenode(n, c, v);
                        close unknownnode(n, c, v, false);
...
lemma void lsg_gap3_helper(struct llnode *from, list<gnode> gnodes, struct llnode *p, int pvalue, struct llnode *c, int v)
    requires lsg(from, 0, gnodes) &*&
        linkex(p, ?pmutex, ?n, ?nmutex, ?nvalue) &*& 
        mem(gnode(p, c, pvalue, v), gnodes) == true &*& 
        mem(gnode(c, n, v, v), gnodes) == true &*&
        sorted(lgn_values(gnodes)) == true &*& 
        pvalue < v &*& v > INT_MIN &*& v < INT_MAX;
    ensures lsg(from, 0, gap3(gnodes, p)) &*& mutex3_held(pmutex) &*& lockednode(c, v) &*&
            ...
            if(gnnode == p) {
                ...
                switch(gnodes2) {
                    ...
                    switch(gn2) {
                        ...
                        open linkex(p, pmutex, n, nmutex, nvalue);
                        open unknownnode(p, c, pvalue, true);
                        open lockednode(p, pvalue);
                        open node_internal(p, _, pvalue);
                        close node_internal(p, pmutex, pvalue);
                        close link(p, n);
    ...
    //@ensures fset(result, nil);
{
    ...
    t->value = INT_MAX;
    t->next = 0;
    //@ close malloc_block_llnode_ex(t);
    //@ close node_internal(t, m, INT_MAX);
    //@ close freenode(t, 0, INT_MAX);
    //@ close unknownnode(t, 0, INT_MAX, false);
    //@ close lsg(0, 0, nil);
    ...
    h->mutex = m;
    mutex3_release(m);
    //@ close node_internal(h, m, INT_MIN);
    //@ close freenode(h, t, INT_MIN);
    //@ close unknownnode(h, t, INT_MIN, false);
    ...
    //@ensures true;
{
    ...
    while(c!=0) 
    //@ invariant (c == 0 ? true : [1/3]c->mutex |-> ?mutex &*& mutex3(mutex) &*& [1/2]c->value |-> ?value) &*& lsg(c, 0, cur_gnodes) &*& ownedOk(cur_gnodes, nil) == true;
    {
        //@ open lsg(c, 0, cur_gnodes);
        //@ open unknownnode(c, ?cn, ?cv, ?co);
        //@ open freenode(c, cn, cv);
        //@ open node_internal(c, _, cv);
...
@*/
/*@
    ensures [f]fset_internal_ex(s, boxId, sh, shmutex) &*& linkex(sh, shmutex, ?shnext, _, _) &*& changeperm(h, boxId, v, b, set_add(owned, gnode(sh, shnext, INT_MIN, v)));
@*/
{
    ...
    @*/
    {
        ...
        //@ assert linkex(p, shmutex, ?pn, ?pnm, ?pnv);
        //@ pnext = pn;        
...
/*@ requires gres(?pvalue) &*& [?f]fset_internal(s, ?boxId) &*& changeperm(?h, boxId, v, ?b, ?owned) &*& 
        linkex(p, ?pmutex, c, ?cmutex, ?cvalue) &*& mutex3_held(cmutex) &*&
...
/*@
    ensures                 
        [f]fset_internal(s, boxId) &*&
        linkex(p, pmutex, c, cmutex, cvalue) &*& 
        linkex(c, cmutex, ?cnext, _, _) &*&
...
@*/
{
    ...
    @*/
    {
        ...
        //@ assert linkex(c, cmutex, ?cn, ?cnm, ?cnv);
        //@ cnext = cn;        
    ...
    //@ close linkex(p, pmutex, c, cmutex, cvalue);
}

void perform_gap1(struct fset* s, int v, struct llnode* p) 
/*@ requires gres(?pvalue) &*& [?f]fset_internal(s, ?boxId) &*& changeperm(?h, boxId, v, ?b, ?owned) &*& 
        linkex(p, ?pmutex, ?c, ?cmutex, ?cvalue) &*& mem(gnode(p, c, pvalue, v), owned) == true &*& v > INT_MIN &*& v < INT_MAX;
@*/
/*@
...
/*@ requires gres(?pvalue) &*& [?f]fset_internal(s, ?boxId) &*& changeperm(?h, boxId, v, ?b, ?owned) &*&
        node_internal(?n, ?nmutex, v) &*& linkex(p, ?pmutex, n, nmutex, v) &*& linkex(n, nmutex, ?c, ?cmutex, ?cvalue) &*& 
...
@*/
/*@
    ensures [f]fset_internal(s, boxId) &*& mutex3_held(pmutex) &*& lockednode(c, v) &*&
...
//@ requires [?f]fset_internal(s, ?boxId) &*& gres(?pvalue) &*& changeperm(?h, boxId, v, ?b, ?owned) &*& linkex(p, ?pm, ?c, ?cm, ?cvalue) &*& mem(gnode(p, c, pvalue, v), owned) == true &*& pvalue < v &*& v <= cvalue;
//@ ensures [f]fset_internal(s, boxId) &*& changeperm(h, boxId, v, b, owned) &*& linkex(p, pm, c, cm, cvalue) &*& b == (cvalue == v);
{
    //@ open gres(pvalue);
    //@ open [f]fset_internal(s, boxId);
    mutex_acquire(s->boxMutex);
...
/*@ ensures [?frac]fset_internal(s, ?boxId) &*& ticket(fset_internal, s, frac) &*& linkex(result, _, ?next, _, ?nvalue) &*&
        gres(?value) &*& changeperm(?h, boxId, v, b, cons(gnode(result, next, value, v), nil)) &*& value < v &*& v <= nvalue &*& b == (v == nvalue);
@*/
{
    //@ open own(s, v, b);
    //@ open [?f] fset_internal(s, ?boxId);
    ...
    //@ open linkex(p, pm, ?pn, ?pnm, ?pnv);
    struct llnode* c = p->next;
    int cvalue = c->value;
    //@ close linkex(p, pm, c, pnm, cvalue);
    //@ close [f] fset_internal(s, boxId);
    while(cvalue < v) 
    /*@ invariant [f]fset_internal(s, boxId) &*& changeperm(?h, boxId, v, b, set_singleton(gnode(p, c, pvalue, v))) &*& 
            linkex(p, ?pmutex, c, ?cmutex, cvalue) &*& v > INT_MIN &*& v < INT_MAX &*& pvalue < v;
    @*/
    {
        //@ open linkex(p, pmutex, c, cmutex, cvalue);
        mutex3_acquire(c->mutex);
        ...
        //@ close linkex(p, pmut, c, cmutex, cvalue);
    ...
    //@ensures own(s, v, result);
{
    ...
    //@ open linkex(p, ?pm, ?cur, ?cm, ?cvalue);
    struct llnode* c = p->next;
    bool result = (c->value == v);
    //@ close linkex(p, pm, c, cm, cvalue);
        
    // save the mutex of p. After performing gap, we no longer have permission to read the mutex of p.        
    //@ open linkex(p, pm, c, cm, cvalue);
    ...
    //@ensures own(s, v, true);
{
    ...
    if(c->value != v) {
        ...
        n->value = v;
        n->next = c;
        //@ close node_internal(n, m, v);
        //@ close linkex(n, m, c, cm, cvalue);
        p->next = n;
        //@ close linkex(p, pmutex, n, m, v);
        ...
        //@ close linkex(p, pmutex, c, cm, cvalue);
        //@ close gres(pvalue);
        perform_gap1(s, v, p);
    }
    ...
    //@ensures own(s, v, false);
{
    ...
    if(c->value == v) {
        ...
        //@ open linkex(c, cm, ?cn, ?cnm, ?cnv);
        struct llnode* n = c->next;
        p->next = n;
        
        //@ close linkex(p, pm, n, cnm, cnv);
        //perform gap3
        //@ close gres(pvalue);
        perform_gap3(s, v, p, c);

        //@ open lockednode(c, v);
        //@ open node_internal(c, _, v);
        mutex3_dispose(c->mutex);
        free(c);
    } else {
        //@ close linkex(p, pmutex, c, cm, cvalue);
