...
//I am in the process of transforming this into pure fractional permissions
...
// The following is a workaround because the theory concerning malloc fails to prove that
...
predicate node_internal(struct llnode *node; struct mutex3 *mutex, int value, int owner, struct llnode *oldNext) =
    node != 0 &*&
    malloc_block_llnode_ex(node) &*&
    [1/3]node->value |-> value &*&
    [1/3]node->mutex |-> mutex &*&
    [1/2]node->owner |-> owner &*&
    [1/2]node->oldNext |-> oldNext;

lemma void node_internal_distinct(struct llnode *n1, struct llnode *n2)
    requires node_internal(n1, ?m1, ?v1, ?o1, ?on1) &*& node_internal(n2, ?m2, ?v2, ?o2, ?on2);
    ensures node_internal(n1, m1, v1, o1, on1) &*& node_internal(n2, m2, v2, o2, on2) &*& n1 != n2;
{
    open node_internal(n1, m1, v1, o1, on1);
    open node_internal(n2, m2, v2, o2, on2);
    malloc_block_llnode_ex_distinct(n1, n2);
    close node_internal(n1, m1, v1, o1, on1);
    close node_internal(n2, m2, v2, o2, on2);
}

predicate link_internal(struct llnode *node; struct mutex3* mutex, struct llnode *next, int value, int owner, struct llnode *oldNext, struct mutex3* nmutex, int nvalue) = 
    node != 0 &*&
    [1/3]node->mutex |-> mutex  &*&
    [1/3]node->value |-> value &*&
    [1/2]node->owner |-> owner  &*&
...
lemma void link_internal_distinct(struct llnode *n1, struct llnode *n2) 
    requires link_internal(n1, ?n1m, ?n1n, ?n1v, ?o1, ?on1, ?n1nm, ?n1nv) &*& link_internal(n2, ?n2m, ?n2n, ?n2v, ?o2, ?on2, ?n2nm, ?n2nv);
    ensures link_internal(n1, n1m, n1n, n1v, o1, on1, n1nm, n1nv) &*& link_internal(n2, n2m, n2n, n2v, o2, on2, n2nm, n2nv) &*& n1 != n2;
{
    open link_internal(n1, n1m, n1n, n1v, o1, on1, n1nm, n1nv);
    open link_internal(n2, n2m, n2n, n2v, o2, on2, n2nm, n2nv);
    close link_internal(n1, n1m, n1n, n1v, o1, on1, n1nm, n1nv);
    close link_internal(n2, n2m, n2n, n2v, o2, on2, n2nm, n2nv);
}

lemma void node_internal_match_link(struct llnode *n)
    requires node_internal(n, ?nm, ?nv, ?no, ?non) &*& link_internal(n, ?nm2, ?nn, ?nv2, ?no2, ?non2, ?nnm, ?nnv);
    ensures node_internal(n, nm, nv, no, non) &*& link_internal(n, nm2, nn, nv2, no2, non2, nnm, nnv) &*& nm == nm2 &*& nv == nv2 &*& no == no2 &*& non == non2;
{
    open node_internal(n, nm, nv, no, non);
    open link_internal(n, nm2, nn, nv2, no2, non2, nnm, nnv);
    close node_internal(n, nm, nv, no, non);
    close link_internal(n, nm2, nn, nv2, no2, non2, nnm, nnv);
}

lemma void node_internal_match_link_next(struct llnode *n)
    requires node_internal(n, ?nm, ?nv, ?no, ?non) &*& link_internal(?p, ?pm, n, ?pv, ?po, ?pon, ?nm2, ?nv2) &*& n!=0;
    ensures node_internal(n, nm, nv, no, non) &*& link_internal(p, pm, n, pv, po, pon, nm2, nv2) &*& nm == nm2 &*& nv == nv2;
{
    open node_internal(n, nm, nv, no, non);
    open link_internal(p, pm, n, pv, po, pon, nm2, nv2);
    close node_internal(n, nm, nv, no, non);
    close link_internal(p, pm, n, pv, po, pon, nm2, nv2);
}

predicate link(struct llnode *node; struct llnode *next, int value, int owner) = 
    link_internal(node, _, next, value, owner, _, _, _); 
...
predicate linkex(struct llnode *node; struct mutex3* mutex, struct llnode *next, int value, int owner, struct mutex3* nmutex, int nvalue) = 
    link_internal(node, mutex, next, value, owner, _, nmutex, nvalue) &*& next != 0; 

predicate linkex2(struct llnode *node; struct mutex3* mutex, struct llnode *next, int value, int owner) = 
    link_internal(node, mutex, next, value, owner, _, _, _);
    ...
    ensures unknownnode(node, next, value, locked) &*& node != 0;
{
    ...
    open node_internal(node, ?m, value, locked, _);
    close node_internal(node, m, value, locked, _);
   ...
   ensures [f]node->mutex |-> mutex &*& mutex3_held(mutex) &*& unknownnode(node, next, value, owner) &*& owner == INT_MIN;
{
    ...
    open node_internal(node, _, value, owner, _);
    close node_internal(node, _, value, owner, _);
    ...
    ensures [f]node->mutex |-> mutex &*& lockednode(node, value, owner) &*& link(node, next, value, owner);
{
    ...
    open node_internal(node, _, value, INT_MIN, _);
    node->owner = owner;
    close node_internal(node, mutex, value, owner, _);
}
    ...
    ensures mutex3_held(mutex) &*& freenode(node, next, value);
{
    ...
    open node_internal(node, _, value, owner2, _);
    node->owner = INT_MIN;
    close node_internal(node, mutex, value, INT_MIN, _);
...
// new predicate instead of unknownnode 
//next is not precise.
predicate node(struct llnode *node; struct mutex3* mutex, struct llnode *next, int value, int owner) =
    node_internal(node, mutex, value, owner, ?oldNext) &*&
    (owner != INT_MIN ? mutex3_held(mutex) &*& next == oldNext : link_internal(node, mutex, next, value, owner, oldNext, _, _));

predicate lsgn(struct llnode *from, struct llnode *to, list<struct llnode *> nodes, list<int> values) =
    switch(values) {
        case nil: 
            return from == to &*& nodes == nil;
        case cons(value, values2): 
            return from != to &*& nodes != nil &*& head(nodes) == from &*& 
                node(from, _, ?next, value, _) &*& 
                lsgn(next, to, ?nodes2, values2) &*&
                nodes == cons(from, nodes2);
    };
    ...
    case cons(gn, gnodes2):
        ...
        case gnode(gnnode, gnnext, gnvalue, gnowner):
            ...
            if(from == node) {
                ...
                open node_internal(node, ?nodemutex, gnvalue, owner, _);
                ...
                close link_internal(node, mutex, gnnext, gnvalue, owner, _, _, _);
    ...
    ensures lsg(from, 0, gnodes) &*& linkex(p, pm, c, pv, o, cm, cv) &*& mem(value, values) == (value == cv);
{
    ...
    open link_internal(p, pm, c, pv, o, _, cm, cv);
    open lsg(c, 0, gns2);
    assert c != 0;
    switch(gns2) {
        case nil:
        ...
        switch(gn) {
            case gnode(gnnode, gnnext, gnvalue, gnowner):
                ...
                open node_internal(c, ?cm2, gnvalue, gnowner, _);
                close linkex(p, pm, c, pv, o, cm, cv);
                close node_internal(c, cm, cv, gnowner, _);
...
lemma void lsg_gap2_helper(struct llnode *from, list<gnode> gnodes, struct llnode *p, int pvalue, struct llnode *c, int v)
    requires 
        ...
        node_internal(n, nmutex, v, INT_MIN, c) &*& 
    ...
    switch(gnodes) {
        ...
        switch(gn) {
            ...
            if(gnnode == p) {
                ...
                open node_internal(p, _, pvalue, v, _);
                open linkex(p, pmutex, n, pvalue, v, nmutex, v);
                open link_internal(p, pmutex, n, pvalue, v, _, nmutex, v);
                open linkex(n, nmutex, c, v, INT_MIN, cmutex, cvalue);
                open link_internal(n, nmutex, c, v, INT_MIN, _, cmutex, cvalue);
                //assert nvalue == v;
                p->owner = INT_MIN;
                close node_internal(p, pmutex, pvalue, INT_MIN, _);
                ...
                switch(gnodes2) {
                    ...
                    switch(gn2) {
                        ...
                        open node_internal(c, _, gn2value, gn2owner, _);
                        close node_internal(c, cmutex, cvalue, gn2owner, _);
    ...
    //@ensures fset(result, nil);
{
    ...
    //@ t->owner = INT_MIN;
    //@ t->oldNext = 0;
    //@ close node_internal(t, m, INT_MAX, INT_MIN, _);
    ...
    //@ h->owner = INT_MIN;
    //@ h->oldNext = t;
    //@ close node_internal(h, m, INT_MIN, INT_MIN, _);
...
@*/
{
    ...
    //@ open link_internal(p, pmutex, c, pvalue, v, _, cmutex, cvalue);
    mutex_acquire(s->boxMutex);
    //@ open fbm_inv(s, boxId)();
...
/*@ requires [?f]fset_internal(s, ?boxId) &*& changeperm(?h, boxId, v, ?b, ?owned) &*&
        node_internal(?n, ?nmutex, v, INT_MIN, ?c) &*& linkex(p, ?pmutex, n, ?pvalue, v, nmutex, v) &*& linkex(n, nmutex, c, v, INT_MIN, ?cmutex, ?cvalue) &*& 
    ...
    //@ensures own(s, v, true);
{
    ...
    if(c->value != v) {
        ...
        //@ n->owner = INT_MIN;
        //@ n->oldNext = c;
        //@ close node_internal(n, m, v, INT_MIN, _);
