...
lemma void count_nonnegative<t>(t x, list<t> xs)
    requires true;
    ensures 0 <= count(x, xs);
{
    switch (xs) {
        case nil:
        case cons(h, t):
            count_nonnegative(x, t);
    }
}

lemma void count_mem<t>(t x, list<t> xs)
    requires true;
    ensures mem(x, xs) == (count(x, xs) > 0);
{
    switch (xs) {
        case nil:
        case cons(h, t):
    ...
    ensures forall(map(snd, es), (count_le)(unit, append(map(snd, es0), cons(x, nil)), append(xs0, cons(x, xs1)))) == true;
{
    switch (es) {
        ...
        case cons(e, es1):
            ...
            count_append(snd(e), map(snd, es0), cons(x, nil));
            count_append(snd(e), xs0, xs1);
            count_append(snd(e), xs0, cons(x, xs1));
            assert count(snd(e), append(map(snd, es0), cons(x, nil))) <= count(snd(e), append(xs0, cons(x, xs1)));
    }
}
    ...
    ensures ghost_list<t>(id, append(xs0, cons(x, xs1))) &*& ghost_list_member_handle<t>(id, x);
{
    ...
    if (mem(x, map(snd, es))) {
        forall_mem(x, map(snd, es), (count_le)(unit, map(snd, es), xs));
        count_append(x, xs0, xs1);
        count_append(x, xs0, cons(x, xs1));
        count_append(x, map(snd, es), cons(x, nil));
    } else {
        count_mem(x, map(snd, es));
        count_nonnegative(x, map(snd, es));
        assert count(x, map(snd, es)) == 0;
        count_append(x, map(snd, es), cons(x, nil));
        count_append(x, xs0, xs1);
        count_append(x, xs0, cons(x, xs1));
        assert count(x, map(snd, es1)) == 1;
        assert count(x, xs2) == count(x, xs0) + count(x, xs1) + 1;
        count_nonnegative(x, xs0);
        count_nonnegative(x, xs1);
        assert count(x, map(snd, es1)) <= count(x, xs2);
    }
    ...
    ensures forall(map(snd, esi), (count_le)(unit, map(snd, append(es0, es1)), append(xs0, xs1))) == true;
{
    switch (esi) {
        ...
        case cons(e, esi0):
            ...
            count_append(snd(e), map(snd, es0), cons(x, map(snd, es1)));
            count_append(snd(e), map(snd, es0), map(snd, es1));
            count_append(snd(e), xs0, cons(x, xs1));
            count_append(snd(e), xs0, xs1);
    }
}
