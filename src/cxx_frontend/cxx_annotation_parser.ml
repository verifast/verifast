...
module Make (Args: Cxx_fe_sig.CXX_TRANSLATOR_ARGS) = struct
  ...
  let make_lexer_token_stream (((start_loc, _), text): raw_annotation) =
    let loc, _, token_stream, _, _ = Lexer.make_lexer_core
      ...
      report_range false false true report_should_fail Lexer.default_file_options.annot_char in
    loc, token_stream
  ...
  let parse_ann_list (anns: raw_annotation list) ann_parser =
    let lexers = anns |> List.map make_lexer_token_stream in
    let current_lexer_nb = ref 0 in
    let nb_lexers = List.length lexers in
    let current_loc =
      if !current_lexer_nb >= nb_lexers then failwith "Lexer index out of range."
      else let c_loc, _ = List.nth lexers !current_lexer_nb in c_loc in
    let rec next_token () =
      let _, stream = List.nth lexers !current_lexer_nb in
      match Lexer.Stream.peek stream with
        | Some (_, Lexer.Eof) | None ->
          if !current_lexer_nb < nb_lexers - 1 then (current_lexer_nb := !current_lexer_nb + 1; next_token ())
          else None
        | Some _ as tk -> Lexer.Stream.junk stream; tk in
