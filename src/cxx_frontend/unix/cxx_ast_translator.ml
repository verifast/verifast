...
let capnp_arr_iter (f: 'a -> 'b) (arr: (Stubs_ast.ro, 'a, R.array_t) Capnp.Array.t): unit =
  arr |> Capnp.Array.iter ~f:f

(**
  [remove_name_qual ?depth name] removes [depth] qualifiers from (qualified) [name].
  All qualifiers are removed if no [depth] is given.

  Examples:
  [remove_name_qual "foo"] -> "foo"
  [remove_name_qual "::foo"] -> "foo"
  [remove_name_qual "::foo::bar"] -> "bar"
  [remove_name_qual ~depth:1 "::foo::bar"] -> "foo::bar"
  [remove_name_qual ~depth:2 "::foo::bar"] -> "bar"
  [remove_name_qual ~depth:10 "::foo::bar"] -> "bar"
*)
let remove_name_qual ?(depth: int = -1) (name: string): string =
  let rec rm_name_qual depth col_count buf name name_length =
    if depth = 0 || name_length = 0 then buf ^ name
    else if col_count = 2 then rm_name_qual (depth - 1) 0 "" name name_length
    else let col_count, buf =
      if name.[0] = ':' then col_count + 1, buf
      else col_count, buf ^ (String.make 1 name.[0]) in
...
module Make (Args: Cxx_fe_sig.CXX_TRANSLATOR_ARGS) : Cxx_fe_sig.Cxx_Ast_Translator = struct
  ...
  *)
  ...
  (**
    [invoke_exporter path allow_expansions] runs the libtool C++ AST exporter. This tool visits each node
    if the C++ AST and serializes it. It also checks if every macro expansion is context free. 
    ...
    In case of success, the libtool first transmits a message {i SerResult.Ok} through {i in_channel} 
    to report that the compilation, context free macro expansion check, and AST serialization was successful.
  ...
  *)
  ...
  and transl_func_like (f: R.Decl.Function.t) = 
    ...
    let return_type = match transl_type_loc @@ result_get f with
    ...
    let transl_param param = 
      ...
      else transl_type_loc @@ type_get param, name_get param in
    let params = params_get f |> capnp_arr_map transl_param in
    name, params, body_opt, (ng_callers_only, ft, pre_post, terminates), return_type
  ...
  and transl_func_decl (loc: VF.loc) (f: R.Decl.Function.t): VF.decl =
    ...
    let body_stmts = match name, body_opt with
  ...
  and transl_record_decl (loc: VF.loc) (record: R.Decl.Record.t): VF.decl list =
    ...
    let record_kind = match kind_get record with
      | R.RecordKind.Struc -> VF.CxxStruct
      | R.RecordKind.Unio -> VF.CxxUnion
      | R.RecordKind.Class -> VF.CxxClass in
    let has_def = has_def_get record in
    ...
    VF.CxxRecord (loc, name, record_kind, body) :: (meths @ decls)
  ...
  and transl_meth_decl (loc: VF.loc) (meth: R.Decl.Method.t): VF.decl =
    ...
    let params = match binding with
      ...
      | VF.Instance -> 
        ...
        (VF.PtrTypeExpr (loc, this_type), "this") :: params in
  ...
  and transl_unary_op_expr (loc: VF.loc) (op: R.Expr.UnaryOp.t): VF.expr =
    ...
    let make_assign loc op lhs post =
      VF.AssignOpExpr (loc, lhs, op, make_int_lit loc 1, post) in
  ...
  and transl_binary_op_expr (loc: VF.loc) (op: R.Expr.BinaryOp.t): VF.expr =
    ...
    let make_op loc op lhs rhs =
      VF.Operation (loc, op, [lhs; rhs]) in
    let make_assign loc op lhs rhs =
      VF.AssignOpExpr (loc, lhs, op, rhs, false) in
    match kind_get op with
      (* binary operators *)
  ...
  and transl_builtin_type (loc: VF.loc) (bt: R.Type.BuiltinKind.t): VF.type_expr = 
    ...
    let make_int signed rank =
      make_man @@ VF.Int (signed, rank) in
    match bt with
  ...
  and transl_record_type (loc: VF.loc) (r: R.Type.Record.t): VF.type_expr =
    ...
    let kind = match kind_get r with
      | R.RecordKind.Struc -> VF.CxxStruct
      | R.RecordKind.Class -> VF.CxxClass
      | R.RecordKind.Unio -> VF.CxxUnion in
    VF.CxxRecordTypeExpr (loc, name, kind)
  ...
  let parse_cxx_file (path: string): Cxx_fe_sig.header_type list * VF.package list =
    ...
    let close_channels () =
      let _ = Unix.close_process_full (inchan, outchan, errchan) in () in
    let on_error () =
    ...
    let try_deser on_receive =
      ...
      match msg with
        | None -> on_error ()
        | Some res -> on_receive res in
    do_finally
      begin fun () ->
        try_deser @@ fun res -> match res |> R.SerResult.of_message |> R.SerResult.get with
