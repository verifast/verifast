open Linemarks_default
open Branch_png

let () =
  ignore @@ GtkMain.Main.init ~setlocale:false ();
  let app = GWindow.window () in
  let branchLeft = Branchleft_png.pixbuf () in
  let branchRight = Branchright_png.pixbuf () in
  ...
  ignore @@ openButton#connect#clicked ~callback:(fun () -> ignore @@ GToolbox.select_file ~title:"Open" ());
  rootVbox#pack ~expand:true scrolledWindow#coerce;
  
  ignore @@ app#connect#destroy ~callback:(fun () -> GMain.Main.quit ());
  app#show ();
  GMain.Main.main ()
