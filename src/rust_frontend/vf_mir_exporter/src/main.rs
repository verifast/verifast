...
use mir_vfmir_transpiler::MirVfmirTranspiler;
...
fn main() {
    let exit_code = rustc_driver::catch_with_exit_code(move || {
        ...
        // TODO: Why it is not able to find sysroot itself
        ...
        // rustc_args.push("--sysroot=".to_owned() + sysroot);

        let mut callbacks = CompilerCalls::default();
        // Call the Rust compiler with our callbacks.
        debug!("Call the Rust Compiler with args: {:?}", rustc_args);
    ...
    fn after_analysis<'tcx>(
        ...
        queries.global_ctxt().unwrap().peek_mut().enter(|tcx| {
            ...
            for (def_id, body) in bodies {
                ...
                MirVfmirTranspiler::transpile(&body.body);
            }
            // mir::pretty::write_mir_pretty(tcx, None, &mut std::io::stderr().lock()).unwrap();
        });
        Compilation::Stop
    }
}
