...
module VerifyProgram(VerifyProgramArgs: VERIFY_PROGRAM_ARGS) = struct
      ...
      let (ftn, ft_predfammaps, fttargs, ftargs) =
        ...
        | Some (ftn, fttargs, args, params, openBraceLoc, ss, closeBraceLoc) ->
          ...
          | Some (ftn, (lft, gh, fttparams, rt, ftxmap, xmap, pre, post, terminates, ft_predfammaps)) ->
            ...
            let fttargs = List.map (fun te -> check_pure_type (pn,ilist) tparams te gh) fttargs in
            let tpenv =
    ...
    | DeclStmt (ld, xs) ->
      let rec iter h tenv ghostenv env xs =
        ...
        | (l, te, x, e, (address_taken, blockPtr))::xs ->
          let t = check_pure_type (pn,ilist) tparams te (if pure then Ghost else Real) in
  ...
  and verify_cont (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env ss cont return_cont econt =
    match ss with
  ...
  and check_backedge_termination currentThread leminfo l tenv h cont =
  ...
  and goto_block (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env return_cont econt block =
  ...
  and verify_block (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env ss cont return_cont econt =
    let (decls, ss) =
