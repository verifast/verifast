  ...
  *)
    ...
    let rec iter map0 map1 =
      ...
      | (cn, (l0,fin0,meths0,fds0,constr0,super0,interfs0,preds0,pn0,ilist0)) as elem::rest ->
        ...
        | Some (l1,fin1,meths1,fds1,constr1,super1,interfs1,preds1,pn1,ilist1) ->
          if fin1 <> fin0 then static_error l1 "Class finality does not match specification.";
  ...
  let rec verify_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env s tcont return_cont =
    ...
    | WhileStmt (l, e, None, dec, ss, closeBraceLoc) ->
      static_error l "Loop invariant required."
    ...
    | WhileStmt (l, e, Some (LoopSpec (pre, post)), dec, ss, closeBraceLoc) ->
      ...
      let return_cont h' retval = assert_false h' [] l "Returning out of a requires-ensures loop is not yet supported." in
      eval_h h' env' e $. fun h' v ->
      begin fun cont ->
        branch
          begin fun () ->
