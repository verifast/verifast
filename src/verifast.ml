  ...
  let rec verify_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env s tcont return_cont econt =
    ...
    | ExprStmt (CallExpr (l, "close_points_to", targs, [], args, Static)) when language = CLang ->
      ...
      begin fun cont ->
        match (targs, args) with
          ([], [LitPat e]) -> cont h env e real_unit_pat real_unit
        | ([], [LitPat e_p; LitPat e_coef]) ->
          let w_coef = check_expr_t (pn,ilist) tparams tenv e_coef RealType in
          eval_h h env w_coef $. fun h env coefTerm ->
          cont h env e_p (TermPat coefTerm) coefTerm
        | _ -> static_error l "close_points_to expects no type argument and one argument." None
      end $. fun h env e coefpat coef ->
      let (w, tp) = check_expr (pn,ilist) tparams tenv e in
      ...
      consume_c_object_core_core l coefpat pointerTerm pointeeType h env true false $. fun _ h (Some value) ->
    ...
    | SwitchStmt (l, e, cs) ->
      ...
      begin match unfold_inferred_type tp with
        ...
        let (tn, targs, Some (_, itparams, ctormap, _, _, _, _, _, _)) = (i, targs, try_assoc' Ghost (pn,ilist) i inductivemap) in
