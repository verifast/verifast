...
pred Tree(node: *mut Node; parent: *mut Node, shape: tree) =
    node != 0 &*&
    std::alloc::alloc_block(node as *u8, std::alloc::Layout::new::<Node>()) &*&
...
pred stack(parent: *mut Node, current: *mut Node, cShape: tree, root: *mut Node, rootShape: tree, stepsLeft: i32, elems_todo: list< *mut u8 >) =
    ...
    if parent == 0 {
        ...
        std::alloc::alloc_block(parent as *u8, std::alloc::Layout::new::<Node>()) &*&
...
impl Tree {
    ...
    //@ req Tree(?root, 0, ?rootShape) &*& x == root &*& x_is_new &*& thread_token(?t) &*& t == currentThread &*& full_borrow(?k, <V>.full_borrow_content(t, visitor)) &*& [?q]lifetime_token(k);
